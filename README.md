...
需开发一套基于C++的 特征数据压缩系统，支持实时压缩BEV特征图  

## 项目内容  
1、BEV特征压缩模块  
实现基于**Eigen矩阵块划分**的压缩算法：将256x256矩阵分块为16x16子矩阵，对每个子矩阵应用 ZFP浮点压缩算法（开源库集成），压缩比可控（默认目标：5:1），支持无损/有损模式，支持Frame_ID快速查找需要的压缩块/子压缩块，并对其进行解压缩；  
2、性能统计与日志  
记录压缩率等指标，通过JSON日志输出运行时统计信息；
